...
/* ASUS BSP audio: change tfa9874 amp fw path +++ */
...
#elif defined ASUS_VODKA_PROJECT
		if (!strncmp(fw_priv->fw_name, "bdwlan", 6)) {
			if (HW_REV_ER2 <= g_ASUS_hwID) {
				snprintf(path, PATH_MAX, "%s/%s", "/vendor/firmware", fw_priv->fw_name);
			} else {
				snprintf(path, PATH_MAX, "%s", "/vendor/firmware/bdwlan_er1.elf");
			}
			dev_err(device, "[wlan] Try to load firmware : %s \n", path);
		}
#else
